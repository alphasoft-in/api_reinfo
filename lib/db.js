...
const initDb = async () => {
    ...
    try {
        ...
        await sql`ALTER TABLE usuarios ADD COLUMN IF NOT EXISTS payment_amount TEXT`;
        await sql`ALTER TABLE usuarios ADD COLUMN IF NOT EXISTS payment_type TEXT`;
        await sql`ALTER TABLE usuarios ADD COLUMN IF NOT EXISTS payment_reference TEXT`;
        ...
        // Migration: Sync existing user quotas with new plan standards
        await sql`UPDATE usuarios SET quota_limit = 5 WHERE plan = 'FREE' AND quota_limit = 100`;
        await sql`UPDATE usuarios SET quota_limit = 5000 WHERE plan = 'PROFESSIONAL' AND quota_limit = 10000`;
        await sql`UPDATE usuarios SET quota_limit = 20000 WHERE plan = 'ENTERPRISE' AND quota_limit = 1000000`;
        await sql`
        ...
        await sql`
            CREATE TABLE IF NOT EXISTS suscripciones (
                ...
                payment_method TEXT DEFAULT 'Transferencia/Banca',
        ...
        `;
        await sql`CREATE INDEX IF NOT EXISTS idx_suscripciones_user_id ON suscripciones(user_id)`;

        // Migration: Seed history for existing users who already have a plan but no history record
        await sql`
            INSERT INTO suscripciones (user_id, plan_name, amount, start_date, end_date, payment_method)
            SELECT 
                u.id, 
                u.plan, 
                CASE 
                    WHEN u.plan = 'PROFESSIONAL' THEN '49' 
                    WHEN u.plan = 'ENTERPRISE' THEN '199' 
                    ELSE '0' 
                END,
                u.created_at,
                COALESCE(u.subscription_end, CURRENT_TIMESTAMP + interval '30 days'),
                'Sincronización Sistema'
            FROM usuarios u
            LEFT JOIN suscripciones s ON u.id = s.user_id
            WHERE s.id IS NULL AND u.role != 'superadmin'
        `;
        ...
        `;
        await sql`CREATE INDEX IF NOT EXISTS idx_notificaciones_user_id ON notificaciones(user_id)`;
        await sql`CREATE INDEX IF NOT EXISTS idx_notificaciones_is_read ON notificaciones(is_read)`;
...
const getRegistros = async (filter, limit = 25, offset = 0) => {
    ...
    if (filter.ruc) whereClause.push(sql`ruc = ${filter.ruc}`);
    if (filter.minero) whereClause.push(sql`minero ILIKE ${'%' + filter.minero + '%'}`);
    if (filter.codigoUnico) whereClause.push(sql`codigo_unico = ${filter.codigoUnico}`);
    if (filter.estado) whereClause.push(sql`estado = ${filter.estado}`);

    const where = whereClause.length > 0 
        ? sql`WHERE ${whereClause.reduce((acc, curr) => sql`${acc} AND ${curr}`)}`
        : sql``;
...
const countRegistros = async (filter) => {
    ...
    const res = await sql`SELECT count(*) FROM registros ${where}`;
...
const adminUpdateUser = async (userId, updates) => {
    const sql = getSql();
    const { plan, quota_limit, subscription_end, active, role, payment_status, requested_plan, payment_amount, payment_type, payment_reference } = updates;
    await sql`
        UPDATE usuarios 
        SET 
            plan = ${plan}, 
...
const deleteUser = async (userId) => {
    ...
    await sql`DELETE FROM suscripciones WHERE user_id = ${userId}`;
    await sql`DELETE FROM usuarios WHERE id = ${userId}`;
};

const addSubscriptionRecord = async (data) => {
    const sql = getSql();
    const { user_id, plan_name, amount, payment_method, start_date, end_date } = data;
    await sql`
        INSERT INTO suscripciones (user_id, plan_name, amount, payment_method, start_date, end_date)
        VALUES (${user_id}, ${plan_name}, ${amount}, ${payment_method}, ${start_date}, ${end_date})
    `;
};

const getUserSubscriptions = async (userId) => {
    const sql = getSql();
    return await sql`SELECT * FROM suscripciones WHERE user_id = ${userId} ORDER BY created_at DESC`;
};
...
module.exports = {
    ...
    addNotification: async (userId, title, message, type = 'info') => {
        const sql = getSql();
        await sql`
            INSERT INTO notificaciones (user_id, title, message, type)
    ...
    markNotificationsAsRead: async (userId) => {
        const sql = getSql();
        await sql`UPDATE notificaciones SET is_read = TRUE WHERE user_id = ${userId}`;
    },
    getSuperadmins: async () => {
        const sql = getSql();
        return await sql`SELECT id FROM usuarios WHERE role = 'superadmin' AND active = TRUE`;
    }
};
