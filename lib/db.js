...
const createUser = async (userData) => {
    const sql = getSql();
    const { username, password, email, role = 'user', plan = 'FREE', quota_limit = 100, subscription_end = null } = userData;
...
const getAllUsers = async () => {
    const sql = getSql();
    return await sql`
        SELECT id, username, email, role, plan, quota_limit, quota_used, subscription_end, active, payment_status, last_payment_at, created_at as "createdAt"
        FROM usuarios 
