    ...
    try {
        ...
        if (parseInt(planesCount[0].count) === 0) {
            await sql`
                ...
                ('Professional', '49', '10,000', '{"10k Consultas RUC", "Exportación Excel/CSV", "API Key dedicada", "Soporte prioritario"}'),
                ('Enterprise', '199', '1,000,000', '{"Consultas Ilimitadas*", "Conexión Directa DB", "Analítica avanzada", "SLA 99.9%", "Manager dedicado"}')
            `;
        }
...
const createUser = async (userData) => {
    ...
    const res = await sql`
        INSERT INTO usuarios (username, password, email, role, api_key, plan, quota_limit, subscription_end)
        VALUES (${username}, ${password}, ${email}, ${role}, ${apiKey}, ${plan}, ${quota_limit}, ${subscription_end})
...
const getAllUsers = async () => {
    const sql = getSql();
    return await sql`
        SELECT id, username, email, role, plan, quota_limit, quota_used, subscription_end, active, payment_status, last_payment_at, two_factor_enabled, created_at as "createdAt"
        FROM usuarios 
...
const adminUpdateUser = async (userId, updates) => {
    const sql = getSql();
    const { plan, quota_limit, subscription_end, active, role, payment_status } = updates;
    await sql`
        UPDATE usuarios 
        SET plan = ${plan}, quota_limit = ${quota_limit}, subscription_end = ${subscription_end}, active = ${active}, role = ${role}, payment_status = ${payment_status}
        WHERE id = ${userId}
    `;
};
